...
class Brats2018Task1Preprocess:
    def __init__(
        ...
        self.grades = ["HGG", "LGG"]    # 뇌종양 등급 (HGG: 악성도가 높음, LGG: 악성도가 낮음)
        self.modalities = ["flair", "t1ce"] # 모달리티 종류

        # 0.7 : 0.15 : 0.15 비율로 분할
    ...
    def _collect_cases_by_grade(self):
        ...
        cases_by_grade = {} # 결과를 저장할 dict ex. {"HGG": [...], "LGG": [...]}
        # 데이터 폴더 구조: 
        # trainset/
        #  - HGG/
        #  - LGG/
        for g in self.grades:   # HGG, LGG 폴더 순회
            grade_dir = os.path.join(self.train_folder_dir, g)  # 현재 grade의 경로 생성 ex. /.../MICCAI_BraTS_2018_Data_Training/HGG
            # 해당 경로 없을 경우 예외처리
            if not os.path.isdir(grade_dir): continue
            # 각 grade에 속하는 case 저장
            grade_cases = []
            for case_name in sorted(os.listdir(grade_dir)):
                case_dir = os.path.join(grade_dir, case_name)   # grade_dir 경로에 현재 파일 경로를 붙임
                if not os.path.isdir(case_dir): continue    # 파일이 없는 경우 예외처리
                seg_fp = self._find_modality_path(case_dir, case_name, "seg")   # 라벨 데이터는 별도로 저장
                if seg_fp is not None:
                    grade_cases.append((g, case_name))  # Seg라벨 데이터가 있으면, grade, case_name 추가 (학습에 사용 가능하다는 의미)
            cases_by_grade[g] = grade_cases # for문으로 grade에 대한 모든 case 탐색이 끝나면 결과 dict에 저장
    ...
    def _split_dataset(self):
        ...
        - 기본은 floor로 배분
        - remainder는 val -> test 순으로 채움
        - 마지막에 val != test면 train에서 부족한 쪽으로 1개 이동해 val == test 강제
        """
        splits = {"train": [], "val": [], "test": []}

        # _collect_cases_by_grade() 에서 저장한 key, value쌍을 튜플 형태로 반환
        # ex. {"HGG": [...], "LGG": [...]}.items() -> [("HGG", [...]), ("LGG", [...])]
        for g, cases in self.cases_by_grade.items():
            n_total = len(cases)    # 전체 데이터 수
            idxs = np.arange(n_total)   # 총 데이터 수만큼 인덱스 번호를 리스트로 저장 ex)np.arange(3) -> [0,1,2]
            np.random.shuffle(idxs)     # 인덱스를 랜덤하게 섞음 [0,1,2] -> [1,0,2]
            ...
            # 안전장치: 합이 정확히 n_total이 되도록 최종 보정
            diff = n_total - (n_train + n_val + n_test)
            if diff != 0:
                # 남으면 train에 더하고, 모자라면 train에서 뺌
                n_train += diff
    ...
    def __len__(self):
        return sum(len(v) for v in self.cases_by_grade.values())

    # ==========================================================
    # 유틸 함수
    # ==========================================================
    # modality 경로를 가져오는 함수
    def _find_modality_path(self, case_dir: str, case_name: str, suffix: str):
        base = os.path.join(case_dir, f"{case_name}_{suffix}")
        for ext in (".nii", ".nii.gz"): # .nii, .nii.gz 모든 확장자에 대해 고려
            fp = base + ext
            if os.path.exists(fp):  # 해당 경로 존재하면 반환
                return fp
        return None

    # modality 정보를 가져오는 함수
    def get_modality_fp(self, grade: str, case_name: str, modality: str):
        case_dir = os.path.join(self.train_folder_dir, grade, case_name)    # 경로 조합
        fp = self._find_modality_path(case_dir, case_name, modality)    # case_name에 해당하는 modality 파일 저장
    ...
    # 파일을 Nibabel의 Nifti 객체 형태로 읽어옴
    # Nifti (Neuroimaging Informatics Technology Initiative): 3D·4D 의료 영상을 저장하기 위한 파일 형식
    def load_nifti(self, fp):
        nifti_data = nibabel.load(fp)   # .nii 파일 열기
        # nifti_data.get_fdata(): numpy array로 MRI 데이터 가져옴, nifti_data.affine: 공간좌표 행렬 가져옴
        return nifti_data.get_fdata(), nifti_data.affine    # 3D이미지 픽셀들, 이미지의 공간적 위치정보(영상이 실제 어느 방향에 놓여 있는지)
    ...
    def preprocess_brats_modality(self, data_fp: str, is_label: bool = False) -> np.ndarray:
        ...
        if is_label:    # 라벨 데이터인 경우
            data = data.astype(np.uint8)
            data[data == 4] = 3    # ET 라벨값을 4에서 3으로 변환
            data = ConvertToMultiChannelBasedOnBrats2018Classes()(data) # 인스턴스 생성 -> (data): 인스턴스 호출 과정을 한 줄로 간소화
        else:
            data = self.normalize(x=data)
            data = data[np.newaxis, ...]    # 새 차원 추가: 채널 추가 → (1, H, W, D)
    ...
    def _process_case(self, grade, case_name):
        ...
        # modality 및 라벨 파일을 가져옴
        flair_fp = self.get_modality_fp(grade, case_name, "flair")
        t1ce_fp = self.get_modality_fp(grade, case_name, "t1ce")
        seg_fp = self.get_modality_fp(grade, case_name, "seg")

        Flair = self.preprocess_brats_modality(flair_fp, is_label=False).swapaxes(1, 3) # (1, H, W, D) → (1, D, W, H)
        T1ce = self.preprocess_brats_modality(t1ce_fp, is_label=False).swapaxes(1, 3) # # (1, H, W, D) → (1, D, W, H)
        Label = self.preprocess_brats_modality(seg_fp, is_label=True).swapaxes(1, 3) # # (1, H, W, D) → (1, D, W, H)

        modalities = np.concatenate((Flair, T1ce), axis=0, dtype=np.float32)
    ...
    # ==========================================================
    def __call__(self):
        print("Started preprocessing BraTS2018 (HGG/LGG, stratified split)...")

        # dict로 저장된 결과를 값들의 리스트 목록으로 불러와서 2중 for문으로 두 리스트 내용을 한 리스트로 합침
        all_cases = [c for g_cases in self.cases_by_grade.values() for c in g_cases]
        with Pool(processes=os.cpu_count()) as pool:
            # cpu 병렬연산을 수행
            pool.starmap(self._process_case, all_cases) # _process_case() 함수를 모든 케이스에 병렬로 적용
...
if __name__ == "__main__":
    brats2018_task1_prep = Brats2018Task1Preprocess(
        ...
        save_dir="/home/work/3D_/seondeok/project/3d_segmentation/SegFormer3D/data/brats2018_seg/BraTS2018_training_data",
        train_split=0.7,
        val_split=0.15,
        test_split=0.15,
    )
