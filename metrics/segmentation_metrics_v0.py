...
class SlidingWindowInference:
    def __init__(self, roi: tuple, sw_batch_size: int):
        ...
        self.post_transform = Compose(
            [
                Activations(softmax=True),  # 수정 <- Activations(sigmoid=True)
                AsDiscrete(argmax=True),    # 수정 <- AsDiscrete(argmax=False, threshold=0.5),
            ]
        )
    ...
    def __call__(
        ...
        gt_post = AsDiscrete(to_onehot=4)   # 추가
        val_labels_list = [gt_post(lbl) for lbl in decollate_batch(val_labels)]   # 수정 <- decollate_batch(val_labels)
